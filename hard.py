hard_questions = [
    {
        "question": "In which year did World War II end?",
        "correct": "1945",
        "wrong": ["1939","1942","1950","1947","1955"]
    },
    {
        "question": "Who was the first person to walk on the Moon?",
        "correct": "Neil Armstrong",
        "wrong": [
            "Buzz Aldrin",
            "Yuri Gagarin",
            "Michael Collins",
            "Alan Bean",
            "Edwin Aldrin"
        ]
    },
    {
        "question": "What does AI stand for?",
        "correct": "Artificial Intelligence",
        "wrong": [
            "Automated Information",
            "Applied Innovation",
            "Advanced Integration",
            "Analog Input",
            "Augmented Insight"
        ]
    },
    {
        "question": "In which year was the US Declaration of Independence signed?",
        "correct": "1776",
        "wrong": [
            "1783",
            "1492",
            "1804",
            "1765",
            "1770"
        ]
    },
    {
        "question": "Who composed the symphony known as the 'Jupiter Symphony'?",
        "correct": "Wolfgang Amadeus Mozart",
        "wrong": [
            "Ludwig van Beethoven",
            "Johann Sebastian Bach",
            "Franz Schubert",
            "Pyotr Ilyich Tchaikovsky"
        ]
    },
    {
        "question": "In which year did the Apollo 11 mission land the first humans on the Moon?",
        "correct": "1969",
        "wrong": [
            "1965",
            "1971",
            "1963",
            "1973"
        ]
    },
    {
        "question": "Which element has the chemical symbol 'W'?",
        "correct": "Tungsten",
        "wrong": [
            "Tantalum",
            "Thorium",
            "Wolfram",
            "Wolframium"
        ]
    },
    {
        "question": "Who is credited with the discovery of penicillin?",
        "correct": "Alexander Fleming",
        "wrong": [
            "Marie Curie",
            "Louis Pasteur",
            "Joseph Lister",
            "Edward Jenner"
        ]
    },
    {
        "question": "Which country hosted the 2008 Summer Olympics?",
        "correct": "China",
        "wrong": [
            "Greece",
            "Australia",
            "United Kingdom",
            "Brazil"
        ]
    },
    {
        "question": "Who won the Academy Award for Best Director in 2019?",
        "correct": "Bong Joon-ho",
        "wrong": [
            "Quentin Tarantino",
            "Martin Scorsese",
            "Sam Mendes",
            "Greta Gerwig"
        ]
    },
    {
        "question": "Which artist painted the 'Mona Lisa'?",
        "correct": "Leonardo da Vinci",
        "wrong": [
            "Vincent van Gogh",
            "Pablo Picasso",
            "Claude Monet",
            "Rembrandt"
        ]
    },
    {
        "question": "In which year did the Titanic sink?",
        "correct": "1912",
        "wrong": [
            "1905",
            "1920",
            "1898",
            "1915"
        ]
    },
    {
        "question": "Who was the first emperor of China?",
        "correct": "Qin Shi Huang",
        "wrong": [
            "Han Wudi",
            "Emperor Wu of Han",
            "Emperor Gaozu of Han",
            "Emperor Taizong of Tang"
        ]
    },
    {
        "question": "Which planet is known as the 'Morning Star'?",
        "correct": "Venus",
        "wrong": [
            "Mars",
            "Mercury",
            "Jupiter",
            "Saturn"
        ]
    },
    {
        "question": "Who wrote the novel '1984'?",
        "correct": "George Orwell",
        "wrong": [
            "Aldous Huxley",
            "Ray Bradbury",
            "H.G. Wells",
            "Isaac Asimov"
        ]
    },
    {
        "question": "Which country was formerly known as Ceylon?",
        "correct": "Sri Lanka",
        "wrong": [
            "Thailand",
            "Myanmar",
            "Nepal",
            "Bangladesh"
        ]
    },
    {
        "question": "Who developed the theory of general relativity?",
        "correct": "Albert Einstein",
        "wrong": [
            "Isaac Newton",
            "Niels Bohr",
            "Marie Curie",
            "Erwin Schr\u00f6dinger"
        ]
    },
    {
        "question": "Which element has the atomic number 79?",
        "correct": "Gold",
        "wrong": [
            "Silver",
            "Platinum",
            "Copper",
            "Iron"
        ]
    },
    {
        "question": "Who was the first female Prime Minister of the United Kingdom?",
        "correct": "Margaret Thatcher",
        "wrong": [
            "Theresa May",
            "Elizabeth II",
            "Indira Gandhi",
            "Golda Meir"
        ]
    },
    {
        "question": "Which country was the first to grant women the right to vote?",
        "correct": "New Zealand",
        "wrong": [
            "United States",
            "United Kingdom",
            "Australia",
            "Finland"
        ]
    },
    {
        "question": "Who invented the telephone?",
        "correct": "Alexander Graham Bell",
        "wrong": [
            "Thomas Edison",
            "Nikola Tesla",
            "Samuel Morse",
            "Guglielmo Marconi"
        ]
    },
    {
        "question": "Which city is known as the 'City of Light'?",
        "correct": "Paris",
        "wrong": [
            "New York",
            "London",
            "Tokyo",
            "Rome"
        ]
    },
    {
        "question": "Who painted the 'Starry Night'?",
        "correct": "Vincent van Gogh",
        "wrong": [
            "Pablo Picasso",
            "Claude Monet",
            "Leonardo da Vinci",
            "Salvador Dal\u00ed"
        ]
    },
    {
        "question": "Who was awarded the Grammy for 'Record of the Year' in 2020?",
        "correct": "Billie Eilish",
        "wrong": [
            "Lizzo",
            "Ariana Grande",
            "Post Malone",
            "Harry Styles"
        ]
    },
    {
        "question": "Which song belongs to Michael Jackson?",
        "correct": "Billie Jean",
        "wrong": [
            "Like a Virgin",
            "I Want It That Way",
            "Purple Rain",
            "Imagine"
        ]
    },
    {
        "question": "Which country won the most Olympic medals in 2008?",
        "correct": "United States",
        "wrong": [
            "China",
            "Russia",
            "Germany",
            "Australia"
        ]
    },
    {
        "question": "Who discovered the rabies vaccine?",
        "correct": "Louis Pasteur",
        "wrong": [
            "Edward Jenner",
            "Alexander Fleming",
            "Robert Koch",
            "Joseph Lister"
        ]
    },
    {
        "question": "Who is known as the founder of Microsoft?",
        "correct": "Bill Gates",
        "wrong": [
            "Steve Jobs",
            "Mark Zuckerberg",
            "Larry Page",
            "Elon Musk"
        ]
    },
    {
        "question": "Which baseball player holds the record for the most career home runs?",
        "correct": "Barry Bonds",
        "wrong": [
            "Hank Aaron",
            "Babe Ruth",
            "Alex Rodriguez",
            "Ken Griffey Jr."
        ]
    },
    {
        "question": "Who won the Nobel Peace Prize in 1993 for the Oslo Accords?",
        "correct": "Yasser Arafat",
        "wrong": [
            "Nelson Mandela",
            "Mother Teresa",
            "Jimmy Carter",
            "Henry Kissinger"
        ]
    },
    {
        "question": "Who was the first woman to receive a Nobel Prize in Physiology or Medicine?",
        "correct": "Gerty Cori",
        "wrong": [
            "Rosalind Franklin",
            "Marie Curie",
            "Barbara McClintock",
            "Dorothy Crowfoot Hodgkin"
        ]
    },
    {
        "question": "Which year did the first Oscar ceremony take place?",
        "correct": "1929",
        "wrong": [
            "1932",
            "1930",
            "1927",
            "1935"
        ]
    },
    {
        "question": "Which country did the first woman astronaut, Valentina Tereshkova, represent?",
        "correct": "Soviet Union",
        "wrong": [
            "United States",
            "China",
            "Germany",
            "United Kingdom"
        ]
    },
    {
        "question": "Who is credited with inventing the first successful airplane?",
        "correct": "Wright brothers",
        "wrong": [
            "Charles Lindbergh",
            "Amelia Earhart",
            "Howard Hughes",
            "Glenn Curtiss"
        ]
    },
    {
        "question": "What year did World War I start?",
        "correct": "1914",
        "wrong": [
            "1912",
            "1915",
            "1939",
            "1905"
        ]
    },
    {
        "question": "Which scientist discovered the electron?",
        "correct": "J.J. Thomson",
        "wrong": [
            "Ernest Rutherford",
            "Niels Bohr",
            "Marie Curie",
            "Albert Einstein"
        ]
    },
    {
        "question": "What is the main ingredient in guacamole?",
        "correct": "Avocado",
        "wrong": [
            "Tomato",
            "Onion",
            "Lemon",
            "Chili"
        ]
    },
    {
        "question": "Which car manufacturer produces the Model S?",
        "correct": "Tesla",
        "wrong": [
            "Ford",
            "Chevrolet",
            "BMW",
            "Audi"
        ]
    },
    {
        "question": "What was the name of the first successful powered flight aircraft?",
        "correct": "Wright Flyer",
        "wrong": [
            "Spirit of St. Louis",
            "Spirit of Innovation",
            "Concorde",
            "Boeing 747"
        ]
    },
    {
        "question": "Which rocket successfully landed humans on the Moon?",
        "correct": "Saturn V",
        "wrong": [
            "Falcon 9",
            "Apollo 11",
            "Soyuz",
            "Space Shuttle"
        ]
    },
    {
        "question": "What is the name of NASA's Mars rover launched in 2020?",
        "correct": "Perseverance",
        "wrong": [
            "Curiosity",
            "Opportunity",
            "Spirit",
            "Insight"
        ]
    },
    {
        "question": "Who was the first human to journey into outer space?",
        "correct": "Yuri Gagarin",
        "wrong": [
            "Neil Armstrong",
            "Buzz Aldrin",
            "Valentina Tereshkova",
            "John Glenn"
        ]
    },
    {
        "question": "What is the term for a spacecraft designed to land on another planet?",
        "correct": "Lander",
        "wrong": [
            "Orbiter",
            "Rover",
            "Probe",
            "Shuttle"
        ]
    },
    {
        "question": "Which company launched the Falcon Heavy rocket?",
        "correct": "SpaceX",
        "wrong": [
            "Blue Origin",
            "NASA",
            "Boeing",
            "Lockheed Martin"
        ]
    },
    {
        "question": "What is the name of the international space station?",
        "correct": "International Space Station (ISS)",
        "wrong": [
            "Hubble Space Telescope",
            "Mir",
            "Chandra X-ray Observatory",
            "Salyut"
        ]
    },
    {
        "question": "Which planet did Voyager 2 visit in 1986?",
        "correct": "Uranus",
        "wrong": [
            "Neptune",
            "Saturn",
            "Jupiter",
            "Mars"
        ]
    },
    {
        "question": "What is the term for the boundary where Earth's atmosphere ends?",
        "correct": "K\u00e1rm\u00e1n Line",
        "wrong": [
            "Stratosphere",
            "Mesosphere",
            "Exosphere",
            "Troposphere"
        ]
    },
    {
        "question": "What is the nickname of New York City?",
        "correct": "The Big Apple",
        "wrong": [
            "The City of Lights",
            "The Windy City",
            "The Sunshine State",
            "The Emerald City"
        ]
    },
    {
        "question": "Which country is nicknamed the 'Land Down Under'?",
        "correct": "Australia",
        "wrong": [
            "New Zealand",
            "South Africa",
            "Canada",
            "United Kingdom"
        ]
    },
    {
        "question": "What is the nickname of the Eiffel Tower?",
        "correct": "Iron Lady",
        "wrong": [
            "The Tower of Lights",
            "The Golden Gate",
            "The Parisian Wonder",
            "The Steel Giant"
        ]
    },
    {
        "question": "What is the nickname of the Sydney Opera House?",
        "correct": "The Sails",
        "wrong": [
            "The Crown",
            "The Shell",
            "The Hut",
            "The Dome"
        ]
    },
    {
        "question": "Which football team is nicknamed 'The Red Devils'?",
        "correct": "Manchester United",
        "wrong": [
            "Liverpool",
            "Arsenal",
            "Chelsea",
            "Barcelona"
        ]
    },
    {
        "question": "What is the nickname of the Amazon Rainforest?",
        "correct": "The Lungs of the Earth",
        "wrong": [
            "The Green Heart",
            "The Heart of Nature",
            "The Wild Frontier",
            "The Earth\u2019s Garden"
        ]
    },
    {
        "question": "Which boxer was nicknamed 'The Greatest'?",
        "correct": "Muhammad Ali",
        "wrong": [
            "Mike Tyson",
            "Joe Frazier",
            "Sugar Ray Leonard",
            "George Foreman"
        ]
    },
    {
        "question": "What is the nickname of the Taj Mahal?",
        "correct": "The Monument of Love",
        "wrong": [
            "The Marble Palace",
            "The White Tomb",
            "The Love Castle",
            "The King's Grave"
        ]
    },
    {
        "question": "Which singer is nicknamed 'The King of Pop'?",
        "correct": "Michael Jackson",
        "wrong": [
            "Elvis Presley",
            "Prince",
            "Freddie Mercury",
            "John Lennon"
        ]
    },
    {
        "question": "Who was the first woman to rule as Pharaoh of Egypt?",
        "correct": "Hatshepsut",
        "wrong": [
            "Cleopatra",
            "Nefertiti",
            "Amina",
            "Merneith"
        ]
    },
    {
        "question": "What was the last major battle of the Napoleonic Wars?",
        "correct": "Battle of Waterloo",
        "wrong": [
            "Battle of Leipzig",
            "Battle of Borodino",
            "Battle of Trafalgar",
            "Battle of Austerlitz"
        ]
    },
    {
        "question": "Which empire was the first to use gunpowder in warfare?",
        "correct": "The Song Dynasty",
        "wrong": [
            "The Roman Empire",
            "The Ottoman Empire",
            "The Mongol Empire",
            "The Byzantine Empire"
        ]
    },
    {
        "question": "Who was the first Roman Emperor?",
        "correct": "Augustus",
        "wrong": [
            "Julius Caesar",
            "Nero",
            "Caligula",
            "Tiberius"
        ]
    },
    {
        "question": "What was the first civilization to develop writing?",
        "correct": "Sumerians",
        "wrong": [
            "Ancient Egyptians",
            "Harappans",
            "Chinese",
            "Olmecs"
        ]
    },
    {
        "question": "Which war marked the end of the Ottoman Empire?",
        "correct": "World War I",
        "wrong": [
            "The Crimean War",
            "The Balkan War",
            "The Napoleonic Wars",
            "The Russo-Turkish War"
        ]
    },
    {
        "question": "Who was the first person to circumnavigate the globe?",
        "correct": "Ferdinand Magellan",
        "wrong": [
            "Christopher Columbus",
            "Marco Polo",
            "Vasco da Gama",
            "James Cook"
        ]
    },
    {
        "question": "What was the first city to host the modern Olympics twice?",
        "correct": "Paris",
        "wrong": [
            "Athens",
            "London",
            "Los Angeles",
            "Beijing"
        ]
    },
    {
        "question": "Who was the last Tsar of Russia?",
        "correct": "Nicholas II",
        "wrong": [
            "Peter the Great",
            "Alexander III",
            "Ivan IV",
            "Alexis I"
        ]
    },
    {
        "question": "What was the first successful powered flight aircraft called?",
        "correct": "The Wright Flyer",
        "wrong": [
            "Spirit of St. Louis",
            "Douglas DC-3",
            "The Kitty Hawk",
            "The Concorde"
        ]
    },
    {
        "question": "Who was the first female Prime Minister of the United Kingdom?",
        "correct": "Margaret Thatcher",
        "wrong": [
            "Theresa May",
            "Elizabeth II",
            "Victoria",
            "Ellen Johnson Sirleaf"
        ]
    },
    {
        "question": "What was the first artificial satellite launched into space?",
        "correct": "Sputnik 1",
        "wrong": [
            "Explorer 1",
            "Vanguard 1",
            "Luna 2",
            "Apollo 11"
        ]
    },
    {
        "question": "Who was the first human to journey into outer space?",
        "correct": "Yuri Gagarin",
        "wrong": [
            "Neil Armstrong",
            "Buzz Aldrin",
            "John Glenn",
            "Valentina Tereshkova"
        ]
    },
    {
        "question": "What was the last major conflict of the Cold War?",
        "correct": "The Soviet-Afghan War",
        "wrong": [
            "The Korean War",
            "The Vietnam War",
            "The Cuban Missile Crisis",
            "The Falklands War"
        ]
    },
    {
        "question": "Who was the first African-American baseball player in Major League Baseball?",
        "correct": "Jackie Robinson",
        "wrong": [
            "Satchel Paige",
            "Willie Mays",
            "Hank Aaron",
            "Roberto Clemente"
        ]
    },
    {
        "question": "What was the first video game ever created?",
        "correct": "Tennis for Two",
        "wrong": [
            "Space Invaders",
            "Pong",
            "Pac-Man",
            "Donkey Kong"
        ]
    },
    {
        "question": "What was the first country to declare independence in the Americas?",
        "correct": "Haiti",
        "wrong": [
            "Mexico",
            "United States",
            "Brazil",
            "Colombia"
        ]
    },
    {
        "question": "Who was the first European explorer to reach India by sea?",
        "correct": "Vasco da Gama",
        "wrong": [
            "Christopher Columbus",
            "Ferdinand Magellan",
            "Marco Polo",
            "Bartolomeu Dias"
        ]
    },
    {
        "question": "What was the last dynasty to rule China before its fall in 1912?",
        "correct": "The Qing Dynasty",
        "wrong": [
            "The Ming Dynasty",
            "The Han Dynasty",
            "The Yuan Dynasty",
            "The Tang Dynasty"
        ]
    },
    {
        "question": "How many triangles are in a Sierpinski triangle after 4 iterations?",
        "correct": "81",
        "wrong": [
            "27",
            "64",
            "256",
            "128"
        ]
    },
    {
        "question": "What is the sum of all angles in a pentagon?",
        "correct": "540\u00b0",
        "wrong": [
            "360\u00b0",
            "180\u00b0",
            "720\u00b0",
            "900\u00b0"
        ]
    },
    {
        "question": "A circle is divided into 8 equal parts. What is the angle of each part?",
        "correct": "45\u00b0",
        "wrong": [
            "90\u00b0",
            "30\u00b0",
            "60\u00b0",
            "120\u00b0"
        ]
    },
    {
        "question": "How many vertices does a dodecahedron have?",
        "correct": "20",
        "wrong": [
            "12",
            "24",
            "16",
            "8"
        ]
    },
    {
        "question": "If a square\u2019s side is doubled, by what factor does its area increase?",
        "correct": "4",
        "wrong": [
            "2",
            "8",
            "16",
            "3"
        ]
    },
    {
        "question": "What is the total number of diagonals in a hexagon?",
        "correct": "9",
        "wrong": [
            "6",
            "12",
            "8",
            "7"
        ]
    },
    {
        "question": "How many lines of symmetry does a regular octagon have?",
        "correct": "8",
        "wrong": [
            "4",
            "6",
            "2",
            "10"
        ]
    },
    {
        "question": "If a rectangle\u2019s length is tripled and its width is halved, what happens to its area?",
        "correct": "It becomes 1.5 times larger",
        "wrong": [
            "It remains the same",
            "It becomes 3 times larger",
            "It becomes 1.5 times smaller",
            "It doubles"
        ]
    },
    {
        "question": "What is the minimum number of colors needed to color a map without adjacent regions sharing the same color?",
        "correct": "4",
        "wrong": [
            "2",
            "5",
            "6",
            "3"
        ]
    },
    {
        "question": "What is the primary language used for web development?",
        "correct": "HTML",
        "wrong": [
            "Python",
            "C",
            "JavaScript",
            "Java"
        ]
    },
    {
        "question": "Which language is known as the 'mother of all programming languages'?",
        "correct": "Fortran/C",
        "wrong": [
            "C++/Pascal",
            "Python",
            "Java/R",
            "Ada/COBOL"
        ]
    },
    {
        "question": "Which language is preferred for machine learning and AI development?",
        "correct": "Python",
        "wrong": [
            "Java",
            "C++",
            "C",
            "Ruby"
        ]
    },
    {
        "question": "Which language is used to query databases?",
        "correct": "SQL",
        "wrong": [
            "Python",
            "Java",
            "C",
            "Ruby"
        ]
    },
    {
        "question": "What is the extension for Python files?",
        "correct": ".py",
        "wrong": [
            ".python",
            ".pth",
            ".pynb",
            ".ipynb"
        ]
    },
    {
        "question": "Which language is known for its speed and efficiency in system programming?",
        "correct": "C",
        "wrong": [
            "Python",
            "Java",
            "C++",
            "Ruby"
        ]
    },
    {
        "question": "What is the primary language used in iOS app development?",
        "correct": "Swift",
        "wrong": [
            "Java",
            "Kotlin",
            "Objective-C",
            "Python"
        ]
    },
    {
        "question": "Which language introduced the concept of object-oriented programming?",
        "correct": "Smalltalk",
        "wrong": [
            "C",
            "C++",
            "Java",
            "Python"
        ]
    },
    {
        "question": "Who wrote The Great Gatsby?",
        "correct": "F. Scott Fitzgerald",
        "wrong": [
            "Ernest Hemingway",
            "William Faulkner",
            "John Steinbeck",
            "J.D. Salinger"
        ]
    },
    {
        "question": "What is the longest novel ever written?",
        "correct": "In Search of Lost Time by Marcel Proust",
        "wrong": [
            "War and Peace by Leo Tolstoy",
            "Don Quixote by Miguel de Cervantes",
            "Ulysses by James Joyce",
            "Moby-Dick by Herman Melville"
        ]
    },
    {
        "question": "Which Shakespeare play features the character Ophelia?",
        "correct": "Hamlet",
        "wrong": [
            "Macbeth",
            "Romeo and Juliet",
            "King Lear",
            "Othello"
        ]
    },
    {
        "question": "What is the central theme of George Orwell\u2019s 1984?",
        "correct": "Totalitarianism and surveillance",
        "wrong": [
            "Freedom and equality",
            "Capitalism",
            "Technological progress",
            "Isolationism"
        ]
    },
    {
        "question": "Who authored One Hundred Years of Solitude?",
        "correct": "Gabriel Garc\u00eda M\u00e1rquez",
        "wrong": [
            "Carlos Fuentes",
            "Mario Vargas Llosa",
            "Jorge Luis Borges",
            "Julio Cort\u00e1zar"
        ]
    },
    {
        "question": "What is the oldest surviving work of literature?",
        "correct": "Epic of Gilgamesh",
        "wrong": [
            "Iliad",
            "Aeneid",
            "Rigveda",
            "Odyssey"
        ]
    },
    {
        "question": "Which artist is known for his drip-painting technique?",
        "correct": "Jackson Pollock",
        "wrong": [
            "Pablo Picasso",
            "Vincent van Gogh",
            "Claude Monet",
            "Salvador Dal\u00ed"
        ]
    },
    {
        "question": "Who wrote Pride and Prejudice?",
        "correct": "Jane Austen",
        "wrong": [
            "Charlotte Bront\u00eb",
            "Emily Bront\u00eb",
            "Virginia Woolf",
            "Elizabeth Barrett Browning"
        ]
    },
    {
        "question": "What is the term for a play that ends tragically?",
        "correct": "Tragedy",
        "wrong": [
            "Comedy",
            "Farce",
            "Melodrama",
            "Musical"
        ]
    },
    {
        "question": "Who composed the Moonlight Sonata?",
        "correct": "Ludwig van Beethoven",
        "wrong": [
            "Wolfgang Amadeus Mozart",
            "Johann Sebastian Bach",
            "Fr\u00e9d\u00e9ric Chopin",
            "Johannes Brahms"
        ]
    },
    {
        "question": "What is the most streamed song on Spotify?",
        "correct": "Blinding Lights by The Weeknd",
        "wrong": [
            "Shape of You by Ed Sheeran",
            "Despacito by Luis Fonsi",
            "Someone Like You by Adele",
            "Bad Guy by Billie Eilish"
        ]
    },
    {
        "question": "Which band released the album Abbey Road?",
        "correct": "The Beatles",
        "wrong": [
            "Pink Floyd",
            "The Rolling Stones",
            "Led Zeppelin",
            "The Who"
        ]
    },
    {
        "question": "Who is known as the 'King of Pop'?",
        "correct": "Michael Jackson",
        "wrong": [
            "Elvis Presley",
            "Prince",
            "Whitney Houston",
            "David Bowie"
        ]
    },
    {
        "question": "What is the highest note a human can sing called?",
        "correct": "Soprano",
        "wrong": [
            "Alto",
            "Tenor",
            "Baritone",
            "Bass"
        ]
    },
    {
        "question": "Which instrument has 88 keys?",
        "correct": "Piano",
        "wrong": [
            "Organ",
            "Guitar",
            "Harpsichord",
            "Accordion"
        ]
    },
    {
        "question": "Who wrote the opera La Traviata?",
        "correct": "Giuseppe Verdi",
        "wrong": [
            "Wolfgang Amadeus Mozart",
            "Giacomo Puccini",
            "Richard Wagner",
            "Giovanni Pergolesi"
        ]
    },
    {
        "question": "What is the term for a group of musicians playing together?",
        "correct": "Ensemble",
        "wrong": [
            "Orchestra",
            "Band",
            "Quartet",
            "Trio"
        ]
    },
    {
        "question": "Which composer is famous for his Symphony No. 9?",
        "correct": "Ludwig van Beethoven",
        "wrong": [
            "Johannes Brahms",
            "Fr\u00e9d\u00e9ric Chopin",
            "Joseph Haydn",
            "W.A. Mozart"
        ]
    },
    {
        "question": "What is the most common time signature in music?",
        "correct": "4/4",
        "wrong": [
            "3/4",
            "2/4",
            "6/8",
            "5/4"
        ]
    },
    {
        "question": "What is the highest attendance ever recorded for a football match?",
        "correct": "199,854",
        "wrong": [
            "160,000",
            "130,000",
            "115,000",
            "140,000"
        ]
    },
    {
        "question": "Which team won the first FIFA World Cup in 1930?",
        "correct": "Uruguay",
        "wrong": [
            "Brazil",
            "Argentina",
            "Germany",
            "Italy"
        ]
    },
    {
        "question": "Who scored the fastest goal in Premier League history?",
        "correct": "Shane Long",
        "wrong": [
            "Robbie Fowler",
            "Ryan Giggs",
            "Cristiano Ronaldo",
            "David Beckham"
        ]
    },
    {
        "question": "Which manager has won the most Premier League titles?",
        "correct": "Sir Alex Ferguson",
        "wrong": [
            "Ars\u00e8ne Wenger",
            "Jos\u00e9 Mourinho",
            "Pep Guardiola",
            "Rafael Ben\u00edtez"
        ]
    },
    {
        "question": "What is the nickname of the Brazilian national football team?",
        "correct": "Sele\u00e7\u00e3o",
        "wrong": [
            "Canarinha",
            "The Blues",
            "The Red Devils",
            "Les Bleus"
        ]
    },
    {
        "question": "Which stadium hosted the 2022 FIFA World Cup final?",
        "correct": "Lusail Iconic Stadium",
        "wrong": [
            "Qatar National Stadium",
            "Al Bayt Stadium",
            "Al Janoub Stadium",
            "Khalifa International Stadium"
        ]
    },
    {
        "question": "Who holds the record for the most runs in Test cricket?",
        "correct": "Sachin Tendulkar",
        "wrong": [
            "Ricky Ponting",
            "Jacques Kallis",
            "Brian Lara",
            "Virat Kohli"
        ]
    },
    {
        "question": "Which venue hosted the 2022 ICC T20 World Cup final?",
        "correct": "Melbourne Cricket Ground",
        "wrong": [
            "Adelaide Oval",
            "The Gabba",
            "Sydney Cricket Ground",
            "Optus Stadium"
        ]
    },
    {
        "question": "What is the maximum number of overs allowed per innings in a T20 match?",
        "correct": "20",
        "wrong": [
            "15",
            "10",
            "25",
            "50"
        ]
    },
    {
        "question": "Which team has reached the most ICC Cricket World Cup finals?",
        "correct": "Australia",
        "wrong": [
            "India",
            "West Indies",
            "New Zealand",
            "Pakistan"
        ]
    },
    {
        "question": "Who scored the fastest century in One Day Internationals?",
        "correct": "AB de Villiers",
        "wrong": [
            "Shahid Afridi",
            "Chris Gayle",
            "Brian Lara",
            "Virat Kohli"
        ]
    },
    {
        "question": "Which bowler has taken the most wickets in Test cricket?",
        "correct": "Muttiah Muralitharan",
        "wrong": [
            "Shane Warne",
            "Anil Kumble",
            "James Anderson",
            "Glenn McGrath"
        ]
    },
    {
        "question": "Which player has the most dismissals as a wicketkeeper in ODIs?",
        "correct": "MS Dhoni",
        "wrong": [
            "Kumar Sangakkara",
            "Adam Gilchrist",
            "Mark Boucher",
            "Ian Healy"
        ]
    },
    {
        "question": "Who hit the first six in T20 World Cup history?",
        "correct": "Shahid Afridi",
        "wrong": [
            "Chris Gayle",
            "Brendon McCullum",
            "Yuvraj Singh",
            "Adam Gilchrist"
        ]
    },
    {
        "question": "Which team won the inaugural ICC T20 World Cup in 2007?",
        "correct": "India",
        "wrong": [
            "Pakistan",
            "Australia",
            "Sri Lanka",
            "New Zealand"
        ]
    },
    {
        "question": "Which cricketer has the nickname 'The Wall'?",
        "correct": "Rahul Dravid",
        "wrong": [
            "Sachin Tendulkar",
            "VVS Laxman",
            "Virat Kohli",
            "Shane Watson"
        ]
    },
    {
        "question": "What is the name of the supermassive black hole at the center of the Milky Way?",
        "correct": "Sagittarius A*",
        "wrong": ["Messier 87*", "Cygnus X-1", "Andromeda A*", "V404 Cygni"]
    },
    {
        "question": "What is the name of the second-largest galaxy in the Local Group?",
        "correct": "Triangulum Galaxy",
        "wrong": ["Andromeda Galaxy", "Whirlpool Galaxy", "Sombrero Galaxy", "Messier 87"]
    },
    {
        "question": "What is the name of the largest moon of Saturn?",
        "correct": "Titan",
        "wrong": ["Europa", "Ganymede", "Callisto", "Triton"]
    },
    {
        "question": "What is the name of the largest moon of Jupiter?",
        "correct": "Ganymede",
        "wrong": ["Titan", "Io", "Callisto", "Europa"]
    },
    {
        "question": "What is the name of the constellation that resembles a hunter?",
        "correct": "Orion",
        "wrong": ["Cassiopeia", "Ursa Major", "Draco", "Lyra"]
    },
    {
        "question": "What is the name of the largest known star?",
        "correct": "UY Scuti",
        "wrong": ["Betelgeuse", "VY Canis Majoris", "Rigel", "Aldebaran"]
    },
    {
        "question": "What is the name of the nearest star to Earth other than the Sun?",
        "correct": "Proxima Centauri",
        "wrong": ["Alpha Centauri A", "Sirius", "Vega", "Betelgeuse"]
    },
    {
        "question": "What is the name of the largest dwarf planet in the Kuiper Belt?",
        "correct": "Pluto",
        "wrong": ["Eris", "Haumea", "Makemake", "Ceres"]
    },
    {
        "question": "What is the name of the comet that appears every 76 years?",
        "correct": "Halley's Comet",
        "wrong": ["Hale-Bopp", "Comet NEOWISE", "Shoemaker-Levy 9", "Comet Encke"]
    },
    {
        "question": "What is the name of the cluster of galaxies that includes the Milky Way?",
        "correct": "Local Group",
        "wrong": ["Virgo Cluster", "Hercules Cluster", "Coma Cluster", "Perseus Cluster"]
    }
]
